...
extern int BUFFER_append_build(BUFFER_HANDLE handle, const unsigned char* source, size_t size);
...
BUFFER_HANDLE_ Base64_Decoder(const char* source)
{
	static BUFFER_ BUFFER_F;
    BUFFER_HANDLE_ result = &BUFFER_F;
    ...
    else
    {
        ...
        {
            if ((result = &BUFFER__/*BUFFER_new()*/) == NULL) // for testing
            {
                /*Codes_SRS_BASE64_06_010: [If there is any memory allocation failure during the decode then Base64_Decode shall return NULL.]*/
                LogError("Could not create a buffer to decoding.");
            }
            else
            {
                ...
                if (sizeOfOutputBuffer > 0)
                {
					memset(result->buffer,NULL,sizeof(result->buffer));
					result->size = sizeOfOutputBuffer;

					Base64decode(result->buffer, source);
                }
            }
        }
    }
...
static STRING_HANDLE_ Base64_Encode_Internal(const unsigned char* source, size_t size)
{
    ...
    else
    {
        ...
        */
		...
		memcpy(result->s,encoded,strlen(encoded));
        if (result == NULL)
        {
            /* free(encoded); */
            LogError("Base64_Encoder:: Allocation failed for return value.");
...
STRING_HANDLE_ Base64_Encode_Bytes(const unsigned char* source, size_t size)
{
...
STRING_HANDLE_ Base64_Encoder(BUFFER_HANDLE_ input)
{
	static STRING_ STRING_F;
    STRING_HANDLE_ result = &STRING_F;
    ...
    else
    {
        ...
        const unsigned char* inputBinary = input->buffer;
