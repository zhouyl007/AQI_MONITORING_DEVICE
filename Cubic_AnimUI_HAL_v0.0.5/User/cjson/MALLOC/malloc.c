...
{  
    ...
    if(ptr==NULL)return;//地址为0.  
 	offset=(u32)ptr-(u32)mallco_dev.membase[SRAMEX];  
    mem_free(SRAMEX,offset);//释放内存     
}  
...
{  
  u32 offset;  									      
	offset=mem_malloc(SRAMEX,size);  	   				   
  if(offset==0XFFFFFFFF)return NULL;  
  else return (void*)((u32)mallco_dev.membase[SRAMEX]+offset);  
}  
...
{  
    ...
    {  									   
	    mymemcpy((void*)((u32)mallco_dev.membase[SRAMEX]+offset),ptr,size);	//拷贝旧内存内容到新内存   
        myfree(/*SRAMEX,*/ptr);  											  		//释放旧内存
        return (void*)((u32)mallco_dev.membase[SRAMEX]+offset);  				//返回新内存首地址
    }  
}
