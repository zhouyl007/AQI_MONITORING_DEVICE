...
static void _DrawIt(void * pData) {
  ...
  #endif
  GUI_FillRect(pDrawItContext->XPos_Text, 
               pDrawItContext->YPos_Text + 55,
               pDrawItContext->XPos_Text + 100,
               pDrawItContext->YPos_Text + 70);
  ...
  //
  GUI_SetColor(GUI_WHITE);
  //GUI_FillPolygon(pDrawItContext->aPointsDest, SIZE_OF_ARRAY(aPoints), 0, 0);
  GUI_FillCircle(120,100,20);
  #endif
  //
  // draw foreground
  //
  ...
  #endif
  GUI_FillRect(pDrawItContext->XPos_Text, 
               70 - pDrawItContext->YPos_Text + 60,
               pDrawItContext->XPos_Text + 100,
               70 - pDrawItContext->YPos_Text + 75);
}
...
static void _DemoBandingMemdev(void) {
  ...
  #endif
  DrawItContext.XPos_Text = 100;

  //GUI_SetColor(GUI_WHITE);
  //GUI_FillCircle(120,120,20);
 //GUI_DrawBitmap(&bmmouth, 120, 100);
  while (1) {
    swap = ~swap;
    for (i = 0; i < 30; i++) {
      //float angle = i * 3.1415926 / 55;
      //DrawItContext.XPos_Text = (swap) ? i : 220 - i;
      DrawItContext.YPos_Text = (swap) ? i : 30 - i;
	  #if 0
      /* Rotate the polygon */
